...
| 脚本 | 阶段 | 功能 | 输入 | 输出 |
|------|------|------|------|------|
| `scripts/douyin_teardown.py` | Stage 1 | 解析抖音分享链接 | 分享URL | Markdown（标题、作者、直链、封面） |
| `scripts/video_analyze.py` | Stage 1 | AI视频内容拆解 | CDN直链 + 保存目录 | Markdown拆解报告（保存到指定目录） |
...
| `scripts/generate_video.py` | Stage 6 | 云梦2.0视频生成 | Prompt文件（单段或分段）+ 可选参考图 | 生成的视频文件(.mp4) |
...
将 Stage 4/5 产出的 Prompt 发送给云梦 2.0 (seedance-2-0) 生成视频。
...
- 分辨率：480p
- 画幅：9:16
- 时长：4-15秒（根据 Segment 时长自动设置）
- 声音：默认开启

### 支持模式

1. **纯 Prompt 生成**：只传文本 Prompt
2. **参考图生成**：传 Prompt + 分镜帧作为参考图（seedance_mode: allround）
3. **首尾帧模式**：传 Prompt + 首帧 + 尾帧（seedance_mode: first_last_frame）

### 脚本使用

```bash
# 单段或多段 Prompt 批量生成
python scripts/generate_video.py "最终视频Prompt.md" --output-dir "qwen_video/20260515/氨糖带货/"

# 带参考图
python scripts/generate_video.py "分段视频Prompt.md" --reference "新分镜_替换后.png" --output-dir "输出目录/"

# 首尾帧模式
python scripts/generate_video.py "prompt.md" --first-frame "首帧.png" --last-frame "尾帧.png" --output-dir "输出目录/"

# 指定时长
python scripts/generate_video.py "prompt.md" --duration 10 --output-dir "输出目录/"
```
