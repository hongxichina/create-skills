...
| `scripts/generate_video.py` | Stage 6 | 云梦2.0视频生成 | Prompt文件 + 分镜参考图 | 生成的视频文件(.mp4) |
...
```bash
# 自动检测时长，逐段生成
python scripts/generate_video.py "分段视频Prompt.md" "新分镜_替换后.png" --output-dir "输出目录/"

# 手动指定时长（单段）
python scripts/generate_video.py "最终视频Prompt.md" "新分镜_替换后.png" --duration 10 --output-dir "输出目录/"
```
